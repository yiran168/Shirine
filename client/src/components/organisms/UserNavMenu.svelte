...
<div id="shirine-user-nav-container" class="relative inline-flex items-center select-none">
  ...
  >
    {#if isGuest}
      <!-- Guest default icon -->
      <div class="w-full h-full flex items-center justify-center text-[var(--on-surface)] group-hover:text-primary transition-colors bg-transparent">
    ...
    >
      ...
      {#if isNormalUser}
        ...
        >
          <span class="flex items-center gap-2.5">
            ...
            <span class="text-xs font-semibold">{authStore.user?.checkedInToday ? t.checkedInToday : t.dailyCheckin}</span>
          </span>
          {#if !authStore.user?.checkedInToday}
        ...
        <div class="h-px bg-[var(--outline-variant)]/15 my-1"></div>
        ...
        >
          ...
          <span class="text-xs font-semibold">{t.signOut}</span>
        </button>
      {/if}
      ...
      {#if isAdmin}
        ...
        >
          ...
          <span class="text-xs font-semibold">{t.adminPanel}</span>
        </a>
