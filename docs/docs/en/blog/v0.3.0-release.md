...
# Shishirinee v0.3.0-rc.1 Prerelease
...
- `@Shishirinee/api`, `@Shishirinee/config`, and `@Shishirinee/ui` are now part of the workspace story
- The frontend no longer depends on Eden Treaty-style calls and instead uses a custom type-safe API client
- Development, deployment, database, and release tasks are grouped under the Shishirinee CLI
...
- Mermaid, KaTeX, callouts, alerts, code copy, image lightbox, and improved markdown image rendeShishirineeg
...
4. Confirm GitHub OAuth variables use `Shishirinee_GITHUB_CLIENT_ID` and `Shishirinee_GITHUB_CLIENT_SECRET`.
...
- Timeline rendeShishirineeg, index scrolling, mobile behavior, and cache initialization issues
